...
<template>
    <draggable :list="list" group="container" item-key="id" v-bind="dragOptions" @update:modelValue="updateList">
        <template #item="{element}">
            <div>
                <div class="rounded shadow-sm my-1 p-2 w-full bg-gray-300 dark:bg-gray-700 dark:text-white cursor-pointer focus:cursor-move hover:bg-slate-300 dark:hover:bg-slate-600 flex flex-row items-center gap-2 group" @click="element.edit()">
                    {{ element.name }}
